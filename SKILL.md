...
8. Run `python3 script/validate.py` before finishing. It deterministically re-derives structure conformance, aggregate statistics, the review_count invariant, Error Index field completeness, the next_unit chain, and (best-effort, via git history) whether aggregate changes are backed by an actual index change. A non-zero exit code means real bugs exist — fix the underlying file(s) and re-run until it exits 0. WARN lines do not block completion but must be read; they flag things (like a word-form judgement call in a collocation) that need a human/AI decision rather than a computed answer. Once the script passes, work through the remaining manual sections of `VALIDATION.md` (error-classification soundness, review-item selection, teaching-quality judgement) before reporting completion to the user. Do not report completion while the script exits non-zero, and do not skip the manual section just because the script passed.
